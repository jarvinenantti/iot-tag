...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    } elseif(strlen(trim($_POST['rfid'])) < 6){
        $rfid_err = "rfid must have at least 6 characters.";
    } else{
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($rfid_err)){
        ...
        $sql = "INSERT INTO user (username, password, rfid) VALUES (?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sss", $param_username, $param_password, $param_rfid);
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group <?php echo (!empty($rfid_err)) ? 'has-error' : ''; ?>">
                <label>rfid</label>
                <input type="rfid" name="rfid" class="form-control" value="<?php echo $rfid; ?>">
